...
public class TargetPreviewManagerTest {
    ...
    String TEST_QUERY_PARAMS =
            "at_preview_params=" + ENCODED_PREVIEW_PARAMS + "&extraKey=extraValue";
    String TEST_CONFIRM_DEEPLINK = "adbinapp://confirm?" + TEST_QUERY_PARAMS;
    String TEST_RESTART_URL = "adbinapp://somepage";

    @Mock private Networking networkService;

    @Mock private UIService uiService;

    @Mock private UriOpening uriService;

    @Mock private Context context;

    @Mock private AssetManager assetManager;

    @Mock private Bitmap floatingButtonImage;

    @Mock private Presentable<FloatingButton> floatingButton;

    @Mock private Presentable<InAppMessage> fullscreenMessage;

    @Mock private HttpConnecting connecting;

    @Mock private TargetPreviewManager previewManager;
    ...
    void runUsingMockedServiceProvider(final Runnable runnable) {
        try (MockedStatic<Base64> base64MockedStatic = Mockito.mockStatic(Base64.class);
                MockedStatic<BitmapFactory> bitmapFactoryMockedStatic =
                        Mockito.mockStatic(BitmapFactory.class);
                MockedStatic<StreamUtils> streamUtilsMockedStatic =
                        Mockito.mockStatic(StreamUtils.class)) {
            base64MockedStatic
                    .when(
                            () ->
                                    Base64.decode(
            ...
            bitmapFactoryMockedStatic
                    .when(() -> BitmapFactory.decodeStream(any(InputStream.class)))
                    .thenReturn(floatingButtonImage);
            streamUtilsMockedStatic
                    .when(() -> StreamUtils.readAsString(any()))
    ...
    void mockFullScreenAndFloatingButton() {
        when(uiService.create(any(FloatingButton.class), any(PresentationUtilityProvider.class)))
                .thenReturn(floatingButton);
        when(uiService.create(any(InAppMessage.class), any(PresentationUtilityProvider.class)))
                .thenReturn(fullscreenMessage);
        ...
        runUsingMockedServiceProvider(
                () -> {
                    ...
                    verify(networkService, times(1))
                            .connectAsync(networkResponseCapture.capture(), any());
                    assertEquals(
                    ...
                    assertEquals(
                            ...
                            2,
                            networkResponseCapture.getValue().getHeaders().size());
                    assertEquals(
                            "network request has the correct request property",
                            "text/html",
                            networkResponseCapture.getValue().getHeaders().get("Accept"));
                    assertEquals(
                            "network request has the correct request property",
                            "application/x-www-form-urlencoded",
                            networkResponseCapture.getValue().getHeaders().get("Content-Type"));
        ...
        runUsingMockedServiceProvider(
                () -> {
                    ...
                    ArgumentCaptor<NetworkRequest> networkResponseCapture =
                            ArgumentCaptor.forClass(NetworkRequest.class);
                    String testDeeplink =
                            "test://path?at_preview_token=olDho0%2Blkfr8LaHURjiCAnu1xIVRkvN3UqdXwawRz3E%3D&key1=val1";
                    String expectedUrl =
                            "https://"
                                    + PREVIEW_DEFAULT_EP
                                    + "/ui/admin/"
                    ...
                    // verify local variables
                    assertEquals(
                            "olDho0+lkfr8LaHURjiCAnu1xIVRkvN3UqdXwawRz3E=",
                            previewManager.getPreviewToken());
                });
    }
    ...
    public void test_EnterPreviewModeWithDeepLinkParams_NullUIService() {
        runUsingMockedServiceProvider(
                () -> {
                    ...
                    String testDeeplink = "test://path?at_preview_token=abcd&key1=val1";
                    previewManager =
                            new TargetPreviewManager(networkService, null, uriService, context);
    ...
    public void test_EnterPreviewModeWithDeepLinkParams_NullUriService() {
        runUsingMockedServiceProvider(
                () -> {
                    ...
                    String testDeeplink = "test://path?at_preview_token=abcd&key1=val1";
                    previewManager =
                            new TargetPreviewManager(networkService, uiService, null, context);
        ...
        runUsingMockedServiceProvider(
                () -> {
                    ...
                    ArgumentCaptor<NetworkRequest> networkResponseCapture =
                            ArgumentCaptor.forClass(NetworkRequest.class);
                    String testDeeplink =
                            "test://path?at_preview_token=abcd&key1=val1&"
                                    + PREVIEW_ENDPOINT
                                    + "=newEndPoint";
                    String expectedUrl =
                            "https://newEndPoint/ui/admin/" + CLIENT_CODE + "/preview?token=abcd";
                    setMockConnectionResponse("TestHTML", 200);
    ...
    public void test_EnterPreviewModeWithDeepLinkParams_LaunchingPreviewModeTwice() {
        runUsingMockedServiceProvider(
                () -> {
                    ...
                    // verify floating button not created again
                    verify(uiService, times(1))
                            .create(
                                    any(FloatingButton.class),
    ...
    @Test
    public void
            test_EnterPreviewModeWithDeepLinkParams_MissingAssetManagerForFloatingButtonImage() {
        runUsingMockedServiceProvider(
                () -> {
                    // setup
    ...
    public void test_previewConfirmedWithUrl_When_Confirm() {
        runUsingMockedServiceProvider(
                () -> {
                    ...
                    // test
                    previewManager.previewConfirmedWithUrl(
                            fullscreenMessage, TEST_CONFIRM_DEEPLINK);

                    // verify
                    verify(fullscreenMessage).dismiss();
                    assertEquals(
                            JSON_PREVIEW_PARAMS,
                            previewManager.getPreviewParameters().replaceAll("\\s+", ""));
                });
    }
    ...
    public void test_previewConfirmedWithUrl_ConfirmPreviewParamsEmpty() {
        runUsingMockedServiceProvider(
                () -> {
                    ...
                    // test
                    previewManager.previewConfirmedWithUrl(
                            fullscreenMessage, "adbinapp://confirm?abc=def");
    ...
    public void test_previewConfirmedWithUrl_InvalidScheme() {
        runUsingMockedServiceProvider(
                () -> {
                    ...
                    // test
                    previewManager.previewConfirmedWithUrl(
                            fullscreenMessage, "notAppScheme://confirm?abc=def");
    ...
    @Test
    public void
            test_previewConfirmedWithUrl_ConfirmPreviewParamsEmpty_Should_Not_Dispatch_TargetResponseEvent() {
        runUsingMockedServiceProvider(
                () -> {
                    // setup
                    previewManager.floatingButtonPresentable = floatingButton;
                    ...
                    // test
                    previewManager.previewConfirmedWithUrl(
                            fullscreenMessage, "adbinapp://confirm?");
    ...
    public void test_fetchWebView_Works() {
        runUsingMockedServiceProvider(
                () -> {
                    ...
                    assertEquals(
                            "network request has the correct URL",
                            "https://someEndpoint/ui/admin/preview",
                            networkResponseCapture.getValue().getUrl());
                    assertEquals(
                            "network request has the correct request property",
                            2,
    ...
    private void verifyFullScreenMessageDisplayed() {
        verify(uiService, times(1))
                .create(any(InAppMessage.class), any(PresentationUtilityProvider.class));
        verify(fullscreenMessage).show();
    }

    private void verifyFloatingButtonDisplayed() {
        verify(uiService, times(1))
                .create(any(FloatingButton.class), any(PresentationUtilityProvider.class));
